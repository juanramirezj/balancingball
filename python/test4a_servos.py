...
while True:
    e = input(f"Enter position servos en grados ({min_position}-{max_position}) or 'q' to quit: ")
    if e == 'q':
        break
    ...
    for j in range(len(ids)):
        servo.MoveTo(ids[j], angulo_to_position(position[j]), wait=False, speed=2400, acc = 50)
