...
# Control gains
Kp     = 0.04 # 0.08 # 0.12 # 0.05 # 0.02   # slightly increased
Ki_pos = 0.000  # reduced — integral was causing windup
Kv     =  0.65 # 0.45  # 0.25 # 0.07 #  0.1
...
for sid in ids:
    servo.SetMode(sid, 0)
    servo.SetAcceleration(sid, 0)  # wsas 200
    servo.SetSpeed(sid, 0)  #was 2400
    servo.StartServo(sid)

cam = Camera()
cam.center_x = 87.0 # 164.0
cam.center_y = 58.0 # 114.0
...
vel_x = vel_y = 0.0
alpha_vel     = 0.82 # 0.75  # 0.85   # slightly less smoothing for faster response
...
while True:
    ...
    # Replace your current 'u =' line with this:
    # We apply Jinv to each component to ensure the axes are correctly mapped
    # --- ADVANCED CONTROL LOGIC ---
    # 1. Calculate components separately
    term_p = Kp * (Jinv @ e_sat)
    term_d = Kv * (Jinv @ v_platform)

    # 2. Limit the P-term so it doesn't "over-tilt" 
    # This ensures the servos always have 'room' to apply braking (Kv)
    P_LIMIT = 6.0 
    mag_p = np.linalg.norm(term_p)
    if mag_p > P_LIMIT:
        term_p *= (P_LIMIT / mag_p)

    # 3. Combine: Pull to center minus the braking force
    u = term_p - term_d
